    ...
    if not st.session_state.students:
        ...
        for idx, (name, data) in enumerate(st.session_state.students.items(), 1):
            with st.expander(f"**{idx}. {name}** - {data['business_type']}", expanded=True):
                ...
                # 대출금 정보 표시
                if data.get('has_loan', False):
                    loan_col1, loan_col2, loan_col3 = st.columns(3)
                    
                    with loan_col1:
                        st.metric("💸 대출 원금", f"{data.get('loan_amount', 0):,}원", 
                                 help="받은 대출 금액")
                    with loan_col2:
                        loan_interest = data.get('loan_interest', 0)
                        st.metric("📈 이자율", f"{loan_interest}%", 
                                 help="정산 시 적용될 이자율")
                    with loan_col3:
                        loan_amount = data.get('loan_amount', 0)
                        repayment = loan_amount * (1 + loan_interest / 100)
                        st.metric("💳 총 상환액", f"{repayment:,.0f}원", 
                                 help=f"원금 {loan_amount:,}원 + 이자 {loan_amount * loan_interest / 100:,.0f}원")
...
with tab4:
    ...
    with tool_tab2:
        ...
        # 게임 밸런스 정보 표시
        st.info("""
        ### 🎯 게임 밸런스 설정 (옵션 2 - 현실성 반영)
        
        **💰 총 시장 자금**: 17,500,000원 (18명 구매자, 2라운드 통합)
        
        **📊 학생별 설정**:
        
        **제조/키트 상품:**
        - 양서준 (헤어스타일링 도구): 원가 7,000원 → 추천 판매가 30,000~50,000원 (마진율 4~7배)
        - 이영미 (폰케이스 꾸미기 키트): 원가 5,000원 → 추천 판매가 20,000~30,000원 (마진율 4~6배)
        
        **의류/패션:**
        - 양성민 (후드집업+하의 세트): 원가 15,000원 → 추천 판매가 40,000~60,000원 (마진율 2.7~4배)
          ※ 상의+하의 세트 가성비 의류, 원가 상향 조정 필요!
        - 박재욱/김태성 (패션 스타일리스트 코디 구독): 원가 20,000원 → 추천 판매가 80,000~120,000원 (마진율 4~6배)
          ※ 전문 스타일리스트 서비스 + 옷 배송, 월 구독제 프리미엄 서비스
        
        **플랫폼/앱 (대출금 있음):**
        - 김범규 (AI 식단관리 앱): 원가 1,000원, **대출금 800,000원** (이자 10%) → 추천 판매가 10,000~20,000원
          ※ 상환액 880,000원, 월 구독 서비스
        - 이소민 (취업 정보 플랫폼): 원가 1,000원, **대출금 1,000,000원** (이자 10%) → 추천 판매가 20,000~30,000원
          ※ 상환액 1,100,000원, 월 구독 서비스
        
        **카페/음식 서비스:**
        - 전찬연 (메이드 카페): 다중상품
          - 오므라이스 2,000원 → 20,000~30,000원 (마진율 10~15배)
          - 메론소다 1,000원 → 10,000~20,000원 (마진율 10~20배)
          - 레인보우샤베트 1,000원 → 10,000~20,000원 (마진율 10~20배)
          - 찬연이의 애교 서비스(가성비) 원가 없음 → 10,000~20,000원
          - 찬연이의 애교 서비스(프리미엄) 원가 없음 → 30,000~50,000원
        
        ---
        
        **👥 구매자 예산 분포** (18명, 2라운드 통합):
        - 💎 큰손 (6명): 1,000,000~2,000,000원 - 품질/프리미엄 서비스 중시
        - 😊 일반 (6명): 400,000~700,000원 - 가성비와 실용성 중시
        - 🤏 짠물 (6명): 150,000~300,000원 - 최저가 추구, 할인 요구
        
        ---
        
        **💡 주요 조정 사항:**
        1. 양성민 원가 3,000 → 15,000원 (옷 세트는 원가가 더 높아야 현실적)
        2. 박재욱/김태성 판매가 40,000~60,000 → 80,000~120,000원 (전문 서비스는 프리미엄 가격)
        3. 이소민 판매가 10,000~20,000 → 20,000~30,000원 (취업 플랫폼은 가치가 높음)
        """)
        ...
        if st.button("📄 구매자 가이드 생성", type="primary"):
            ...
            **👥 총 구매자**: {total_buyers}명  
            ...
            # 구매자별 예산 및 구매 전략 가이드
            st.markdown("## 💰 구매자별 예산 및 구매 전략")
            st.caption("2라운드에 걸쳐 사용할 총 예산입니다. 전략적으로 배분하세요!")
            
            # 큰손 가이드
            st.markdown("### 💎 큰손 구매자 (6명)")
            
            big_spender_guide = st.container()
            with big_spender_guide:
                col1, col2 = st.columns(2)
                
                with col1:
                    st.metric("💰 총 예산 (2라운드)", "1,000,000~2,000,000원")
                with col2:
                    st.metric("📊 권장 사용률", "70-90%")
                
                st.info("""
                **💭 예산 마인드**: 돈은 충분하니 가치를 본다
                
                **🎯 구매 의사결정 기준**:
                - ✅ 1순위: 품질과 독창성 (가격은 중요하지 않음)
                - ✅ 2순위: 사업의 확장 가능성, 투자 가치
                - ✅ 3순위: 개인적인 취향, 감성적 만족
                - ❌ 거부: 저품질, 대량생산 느낌, 성의없는 제품
                
                **💵 가격 기준**: 원가의 2~3배까지 OK  
                **💬 가격 흥정**: 안 함 (품질 좋으면 즉시 구매)  
                **🛍️ 구매 패턴**: 한 번에 여러 개 구매 가능, 고가 상품 선호  
                **👥 추천**: 3-5명의 학생에게서 구매
                """)
            ...
            # 일반 가이드
            st.markdown("### 😊 일반 구매자 (6명)")
            
            normal_guide = st.container()
            with normal_guide:
                col1, col2 = st.columns(2)
                
                with col1:
                    st.metric("💰 총 예산 (2라운드)", "400,000~700,000원")
                with col2:
                    st.metric("📊 권장 사용률", "70-85%")
                
                st.info("""
                **💭 예산 마인드**: 합리적인 소비를 추구한다
                
                **🎯 구매 의사결정 기준**:
                - ✅ 1순위: 가성비 (가격 대비 품질)
                - ✅ 2순위: 실용성과 필요성
                - ✅ 3순위: 다른 판매처와 비교
                - ❌ 거부: 과도하게 비쌈, 쓸모없음, 품질 의심
                
                **💵 가격 기준**: 원가의 1.5~2배가 적정  
                **💬 가격 흥정**: 정중하게 가격 문의 가능 (10% 할인 요청)  
                **🛍️ 구매 패턴**: 신중하게 선택, 2-3군데 비교 후 결정  
                **👥 추천**: 2-4명의 학생에게서 구매
                """)
            ...
            # 짠물 가이드
            st.markdown("### 🤏 짠물 구매자 (6명)")
            
            frugal_guide = st.container()
            with frugal_guide:
                col1, col2 = st.columns(2)
                
                with col1:
                    st.metric("💰 총 예산 (2라운드)", "150,000~300,000원")
                with col2:
                    st.metric("📊 권장 사용률", "60-80%")
                ...
                **🎯 구매 의사결정 기준**:
                - ✅ 1순위: 최저가 (무조건 싼 것)
                - ✅ 2순위: 할인 가능 여부
                - ✅ 3순위: 정말 꼭 필요한가? (구매 망설임)
                - ❌ 거부: 비쌈, 불필요함, 사치품
                
                **💵 가격 기준**: 원가의 1~1.5배만 지불  
                **💬 가격 흥정**: 적극적으로 가격 흥정 (20-30% 할인 요구)  
                **🛍️ 구매 패턴**: 최소한만 구매, 여러 번 고민 후 결정  
                **👥 추천**: 1-2명의 학생에게서 구매
                """)
            ...
            if not st.session_state.students:
                ...
                for name, data in st.session_state.students.items():
                    ...
                    # 학생 정보 표시 (대출금 포함)
                    student_info = f"**{name}** - {business_type} (원가: {cost:,}원)"
                    if data.get('has_loan', False):
                        loan_amount = data.get('loan_amount', 0)
                        loan_interest = data.get('loan_interest', 0)
                        repayment = loan_amount * (1 + loan_interest / 100)
                        student_info += f" 💸 대출금: {loan_amount:,}원 (상환액: {repayment:,.0f}원)"
                    
                    with st.expander(student_info):
                        # 모든 캐릭터의 예상 가격대 계산
                        st.markdown("#### 💎 큰손 구매자들")
                        big_df_data = []
            ...
            for i, character in enumerate(selected_big_spenders, 1):
                with st.expander(f"{character['emoji']} 큰손 #{i}: {character['name']}"):
                    st.write(f"""
                    **👤 캐릭터**: {character['name']}  
                    **💰 총 예산 (2라운드)**: {character['budget']}  
                    **🎯 특성**: {character['personality']}  
                    **📋 구매 조건**: **원가의 2~3배** 가격까지 구매  
                    **💬 말투 예시**:  
                    - "{character['speech'][0]}"  
                    - "{character['speech'][1]}"  
                    - "{character['speech'][2]}"  
